...
DEBUG.style.cssText = 'position:fixed;bottom:76px;left:8px;right:8px;background:#111;color:#4ec9b0;font:11px monospace;padding:6px 10px;z-index:999;max-height:60px;overflow-y:auto;border-radius:6px;border:1px solid #2a2a2a;opacity:0.92;';
DEBUG.id = 'debug-log';
const DBG_CLOSE = document.createElement('span');
DBG_CLOSE.textContent = '×';
DBG_CLOSE.style.cssText = 'position:absolute;top:2px;right:8px;cursor:pointer;color:#6b6b6b;font-size:14px;';
DBG_CLOSE.onclick = function() { DEBUG.style.display = 'none'; };
DEBUG.appendChild(DBG_CLOSE);
const DBG_TEXT = document.createElement('pre');
DBG_TEXT.style.cssText = 'margin:0;white-space:pre-wrap;word-break:break-all;';
...
function debug(msg) {
  ...
  if (msg.includes('WS onopen fired')) {
    _dbgTimeout = setTimeout(function() { DEBUG.style.display = 'none'; }, 3000);
  }
  ...
  DBG_TEXT.textContent += '[' + new Date().toLocaleTimeString() + '] ' + msg + '\n';
